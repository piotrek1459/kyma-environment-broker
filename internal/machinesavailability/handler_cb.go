...
func (h *HandlerCB) getSecret(provider string) (*unstructured.Unstructured, error) {
	...
	credentialsBinding, err := h.getCredentialsBindingForRule(provider, matchedRule)
	if err != nil {
		return nil, err
	}
...
func (h *HandlerCB) getCredentialsBindingForRule(provider string, matchedRule rules.Result) (*gardener.CredentialsBinding, error) {
	...
	v := &gardener.SecretAccessValidator{GardenerClient: h.gardenerClient}
