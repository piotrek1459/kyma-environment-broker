...
func TestUpdateEndpoint_UpdateSuspension(t *testing.T) {
	...
	svc := broker.NewUpdate(
		...
		kcBuilder,
		fakeKcpK8sClient,
		nil, nil, imConfigFixture, newSchemaService(t), nil, nil, nil, nil, nil, nil, blocklist.OperationBlocklist{})
...
func TestUpdateEndpoint_UpdateOfExpiredTrial(t *testing.T) {
	...
	svc := broker.NewUpdate(broker.Config{}, st, handler, true, false, true, q, broker.PlansConfig{},
		nil, fixLogger(),
		dashboardConfig, kcBuilder, fakeKcpK8sClient, nil, nil, imConfigFixture, newSchemaService(t), nil, nil, nil, nil, nil, nil, blocklist.OperationBlocklist{})
...
func TestUpgradePlan(t *testing.T) {
	...
	svc := broker.NewUpdate(broker.Config{
		...
		dashboardConfig, kcBuilder, fakeKcpK8sClient, newProviderSpec(t), newPlanSpec(t), imConfigFixture, newSchemaService(t), nil, nil, nil, nil, nil, nil, blocklist.OperationBlocklist{})
...
func TestUpdateEndpoint_UpdateAutoscalerParams(t *testing.T) {
	...
	svc := broker.NewUpdate(broker.Config{}, st, handler, true, false, true, q, broker.PlansConfig{},
		...
		fakeKcpK8sClient, newProviderSpec(t), newPlanSpec(t), imConfigFixture, newSchemaService(t), nil, nil, nil, nil, nil, nil, blocklist.OperationBlocklist{})
...
func TestUpdateEndpoint_UpdateInstanceWithWrongActiveValue(t *testing.T) {
	...
	svc := broker.NewUpdate(broker.Config{}, st, handler, true, false, true, q, broker.PlansConfig{},
		nil, fixLogger(), dashboardConfig, kcBuilder,
		fakeKcpK8sClient, nil, nil, imConfigFixture, newSchemaService(t), nil, nil, nil, nil, nil, nil, blocklist.OperationBlocklist{})
...
func TestUpdateEndpoint_UpdateGlobalAccountID(t *testing.T) {
	...
	svc := broker.NewUpdate(broker.Config{}, st, handler, true, true, false, q, broker.PlansConfig{},
		nil, fixLogger(), dashboardConfig, kcBuilder, fakeKcpK8sClient, nil, nil, imConfigFixture, newSchemaService(t), nil, nil, nil, nil, nil, nil, blocklist.OperationBlocklist{})
...
func TestUpdateAdditionalWorkerNodePools(t *testing.T) {
	for tn, tc := range map[string]struct {
		...
		"Valid taints": {
			additionalWorkerNodePools: `[{"name": "name-1", "machineType": "m6i.large", "haZones": true, "autoScalerMin": 3, "autoScalerMax": 20, "taints": [{"key": "dedicated", "value": "gpu", "effect": "NoSchedule"}, {"key": "spot", "value": "true", "effect": "PreferNoSchedule"}]}]`,
			expectedError:             false,
		},
		"Valid taints with duplicate key and different effect": {
			additionalWorkerNodePools: `[{"name": "name-1", "machineType": "m6i.large", "haZones": true, "autoScalerMin": 3, "autoScalerMax": 20, "taints": [{"key": "dedicated", "value": "gpu", "effect": "NoSchedule"}, {"key": "dedicated", "value": "gpu", "effect": "NoExecute"}]}]`,
			expectedError:             false,
		},
		"Empty taint key": {
			additionalWorkerNodePools: `[{"name": "name-1", "machineType": "m6i.large", "haZones": true, "autoScalerMin": 3, "autoScalerMax": 20, "taints": [{"key": "", "value": "gpu", "effect": "NoSchedule"}]}]`,
			expectedError:             true,
		},
		"Empty taint object": {
		...
		"Invalid taint effect": {
			additionalWorkerNodePools: `[{"name": "name-1", "machineType": "m6i.large", "haZones": true, "autoScalerMin": 3, "autoScalerMax": 20, "taints": [{"key": "dedicated", "value": "gpu", "effect": "InvalidEffect"}]}]`,
			expectedError:             true,
		},
		"Duplicate taint key and effect": {
			additionalWorkerNodePools: `[{"name": "name-1", "machineType": "m6i.large", "haZones": true, "autoScalerMin": 3, "autoScalerMax": 20, "taints": [{"key": "dedicated", "value": "gpu", "effect": "NoSchedule"}, {"key": "dedicated", "value": "other", "effect": "NoSchedule"}]}]`,
			expectedError:             true,
		},
...
func TestUpdateInternalOnlyMachinesForExternalCustomer(t *testing.T) {
	for tn, tc := range map[string]struct {
		planID                    string
		additionalWorkerNodePools string
		expectedError             string
	}{
		"single AWS internal-only machine (prefix g6)": {
			planID:                    broker.AWSPlanID,
			additionalWorkerNodePools: additionalWorkerNodePools,
			expectedError:             "The following machine types: g6.xlarge (used in worker node pools: name-1) are not available for your account. For details, please contact your sales representative.",
		},
		"multiple AWS internal-only machines (prefix g6)": {
			planID:                    broker.AWSPlanID,
			...
			expectedError:             "The following machine types: g6.xlarge (used in worker node pools: name-1), g6.2xlarge (used in worker node pools: name-2) are not available for your account. For details, please contact your sales representative.",
		},
		"duplicate AWS internal-only machine (prefix g6)": {
			planID:                    broker.AWSPlanID,
			...
			expectedError:             "The following machine types: g6.xlarge (used in worker node pools: name-1, name-2) are not available for your account. For details, please contact your sales representative.",
		},
		"single AWS internal-only machine (prefix g4dn)": {
			planID:                    broker.AWSPlanID,
			additionalWorkerNodePools: `[{"name": "name-1", "machineType": "g4dn.xlarge", "haZones": true, "autoScalerMin": 3, "autoScalerMax": 20}]`,
			expectedError:             "The following machine types: g4dn.xlarge (used in worker node pools: name-1) are not available for your account. For details, please contact your sales representative.",
		},
		"single GCP internal-only machine (prefix g2-standard)": {
			planID:                    broker.GCPPlanID,
			additionalWorkerNodePools: `[{"name": "name-1", "machineType": "g2-standard-4", "haZones": true, "autoScalerMin": 3, "autoScalerMax": 20}]`,
			expectedError:             "The following machine types: g2-standard-4 (used in worker node pools: name-1) are not available for your account. For details, please contact your sales representative.",
		},
		"single Azure internal-only machine (prefix Standard_NC)": {
			planID:                    broker.AzurePlanID,
			additionalWorkerNodePools: `[{"name": "name-1", "machineType": "Standard_NC4as_T4_v3", "haZones": true, "autoScalerMin": 3, "autoScalerMax": 20}]`,
			expectedError:             "The following machine types: Standard_NC4as_T4_v3 (used in worker node pools: name-1) are not available for your account. For details, please contact your sales representative.",
		},
	} {
		t.Run(tn, func(t *testing.T) {
			// given
...
func TestAvailableZonesValidationDuringUpdate(t *testing.T) {
	...
	svc := broker.NewUpdate(broker.Config{}, st, handler, true, true, false, q, broker.PlansConfig{},
		fixValueProvider(t), fixLogger(), dashboardConfig, kcBuilder, fakeKcpK8sClient, newProviderSpec(t), newPlanSpec(t), imConfig, newSchemaService(t), nil, nil, nil, nil, nil, nil, blocklist.OperationBlocklist{})
...
func TestQuotaLimitCheckDuringUpdate(t *testing.T) {
	...
	t.Run("should create new operation if there is no other instances", func(t *testing.T) {
		...
		svc := broker.NewUpdate(broker.Config{
			...
			fixValueProvider(t), fixLogger(),
			dashboardConfig, kcBuilder, fakeKcpK8sClient, newProviderSpec(t), newPlanSpec(t), imConfigFixture, newSchemaService(t), quotaClient, nil, nil, nil, nil, nil, blocklist.OperationBlocklist{})
	...
	t.Run("should create new operation if there is no unassigned quota but whitelisted subaccount", func(t *testing.T) {
		...
		svc := broker.NewUpdate(broker.Config{
			...
			fixValueProvider(t), fixLogger(),
			dashboardConfig, kcBuilder, fakeKcpK8sClient, newProviderSpec(t), newPlanSpec(t), imConfigFixture, newSchemaService(t), quotaClient, whitelist.Set{subAccountID: struct{}{}}, nil, nil, nil, nil, blocklist.OperationBlocklist{})
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			svc := broker.NewUpdate(broker.Config{}, st, handler, true, true, false, q, broker.PlansConfig{},
				...
				rulesService, fixture.CreateGardenerClientWithCredentialsBindings(), fixture.NewFakeFactory(tc.zones, tc.awsError), blocklist.OperationBlocklist{})
	...
	for tn, tc := range map[string]struct {
		...
		t.Run(tn, func(t *testing.T) {
			...
			svc := broker.NewUpdate(brokerCfg, st, handler, true, true, false, q, broker.PlansConfig{},
				...
				nil, nil, tc.gvisorWhitelist, nil, nil, nil, blocklist.OperationBlocklist{})
...
func TestUpdateBlocklist(t *testing.T) {
	...
	newUpdateSvc := func(t *testing.T, st storage.BrokerStorage, bl blocklist.OperationBlocklist) *broker.UpdateEndpoint {
		q := &automock.Queue{}
		q.On("Add", mock.AnythingOfType("string"))
		kcBuilder := &kcMock.KcBuilder{}
		return broker.NewUpdate(broker.Config{}, st, &handler{}, true, false, true, q, broker.PlansConfig{},
			nil, fixLogger(), dashboardConfig, kcBuilder, fakeKcpK8sClient,
			nil, nil, imConfigFixture, newSchemaService(t), nil, nil, nil, nil, nil, nil, bl)
	}

	t.Run("update is blocked for aws plan", func(t *testing.T) {
		// given
		st := storage.NewMemoryStorage()
		instance := internal.Instance{
			InstanceID:    instanceID,
		...
		// then
		require.Error(t, err)
		assert.Contains(t, err.Error(), "aws updates are blocked")
	})

	t.Run("update is allowed for non-blocked plan", func(t *testing.T) {
		// given
		st := storage.NewMemoryStorage()
		instance := internal.Instance{
			InstanceID:    instanceID,
