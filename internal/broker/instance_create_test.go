...
func TestAdditionalWorkerNodePools(t *testing.T) {
	for tn, tc := range map[string]struct {
		...
		"Valid taints": {
			additionalWorkerNodePools: `[{"name": "name-1", "machineType": "m6i.large", "haZones": true, "autoScalerMin": 3, "autoScalerMax": 20, "taints": [{"key": "t1", "value": "v1", "effect": "NoSchedule"}]}]`,
			expectedError:             false,
		},
		"Multiple valid taints": {
			additionalWorkerNodePools: `[{"name": "name-1", "machineType": "m6i.large", "haZones": true, "autoScalerMin": 3, "autoScalerMax": 20, "taints": [{"key": "t1", "value": "v1", "effect": "NoSchedule"}, {"key": "t2", "value": "v2", "effect": "PreferNoSchedule"}]}]`,
			expectedError:             false,
		},
		"Taint with empty value": {
			additionalWorkerNodePools: `[{"name": "name-1", "machineType": "m6i.large", "haZones": true, "autoScalerMin": 3, "autoScalerMax": 20, "taints": [{"key": "t1", "value": "", "effect": "NoExecute"}]}]`,
			expectedError:             false,
		},
		"Taint with empty key": {
			additionalWorkerNodePools: `[{"name": "name-1", "machineType": "m6i.large", "haZones": true, "autoScalerMin": 3, "autoScalerMax": 20, "taints": [{"key": "", "value": "v1", "effect": "NoSchedule"}]}]`,
			expectedError:             true,
		},
		"Taint with invalid effect": {
		...
		"Duplicate taints same key and effect": {
			additionalWorkerNodePools: `[{"name": "name-1", "machineType": "m6i.large", "haZones": true, "autoScalerMin": 3, "autoScalerMax": 20, "taints": [{"key": "t1", "value": "v1", "effect": "NoSchedule"}, {"key": "t1", "value": "v2", "effect": "NoSchedule"}]}]`,
			expectedError:             true,
		},
...
func newSchemaService(t *testing.T) *broker.SchemaService {
	...
	schemaService := broker.NewSchemaService(provider, plans, nil, broker.Config{},
		broker.StringList{broker.TrialPlanName, broker.AzurePlanName, broker.AzureLitePlanName, broker.AWSPlanName,
			broker.GCPPlanName, broker.SapConvergedCloudPlanName, broker.FreemiumPlanName}, channelResolver, nil)
...
func TestProvisionInternalOnlyMachinesForExternalCustomer(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:                      "single AWS internal-only machine (prefix g6)",
			planID:                    broker.AWSPlanID,
			additionalWorkerNodePools: `[{"name": "name-1", "machineType": "g6.xlarge", "haZones": true, "autoScalerMin": 3, "autoScalerMax": 20}]`,
		...
		{
			name:                      "multiple AWS internal-only machines (prefix g6)",
			planID:                    broker.AWSPlanID,
		...
		{
			name:                      "duplicate AWS internal-only machine (prefix g6)",
			planID:                    broker.AWSPlanID,
		...
		{
			name:                      "single AWS internal-only machine (prefix g4dn)",
			planID:                    broker.AWSPlanID,
		...
		{
			name:                      "single GCP internal-only machine (prefix g2-standard)",
			planID:                    broker.GCPPlanID,
		...
		{
			name:                      "single Azure internal-only machine (prefix Standard_NC)",
			planID:                    broker.AzurePlanID,
