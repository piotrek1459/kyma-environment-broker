...
According to the OSB API specification, the update response may be synchronous and return the HTTP `200` status code. It can happen when the parameters aren't modified.

> ### Note:
The process for updating SAP Service Manager credentials is different. The credentials are stored in the Kyma Environment Broker (KEB) database, and [Runtime Reconciler](../contributor/07-10-runtime-reconciler.md) updates them during reconciliation.
