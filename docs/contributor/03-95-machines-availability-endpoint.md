...
and whether those machine types support high availability (HA) in these regions.
...
The list of machine types and regions originates from Kyma Environment Broker’s (KEB) [provider configuration](02-60-plan-configuration.md). 
...
If a machine type is supported in at least three availability zones within that region, it is marked with `high_availability`.
