...
The Job iterates over the instances, and for each one with `deletedAt` appropriately set, sends a DELETE request to Kyma Environment Broker (KEB).  

## Prerequisites

* The KEB database to get the IDs of instances with not completed steps
...
> In this mode, the Job only logs the information about candidate instances, that is, instances meeting the configured criteria. The instances are not affected.
